...
class SemanticRuntime:
    """Privacy-preserving local embeddings plus optional LiteLLM query planning."""
    ...
    def _read_planner_key(self) -> str:
        ...
        try:
            ...
            if metadata.st_uid != os.getuid() or stat.S_IMODE(metadata.st_mode) & 0o077:
                raise PermissionError("planner key file must be owner-only")
            value = os.read(descriptor, 8192).decode().strip()
        finally:
            os.close(descriptor)
        if not value or len(value) > 4096:
            raise ValueError("planner key file is invalid")
        return value
